<?php
function agregar_filtros_noticias() {
...
function filtrar_noticias($query) {
...
function modificar_url_noticias($url, $post) {

    if ($post->post_type === 'noticias') {
        // Obtener el término de la taxonomía asociada
        $terms = wp_get_post_terms($post->ID, 'categoria_noticias');
        if ($terms) {
            // Reemplazar el marcador de posición en la URL por el término
            $url = str_replace('%categoria_noticias%', $terms[0]->slug, $url);
        }
    }
    return $url;
...
add_action('restrict_manage_posts', 'agregar_filtros_noticias');
add_action('pre_get_posts', 'filtrar_noticias');
add_filter('post_type_link', 'modificar_url_noticias', 10, 2);
